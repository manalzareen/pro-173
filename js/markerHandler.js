...
AFRAME.registerComponent("markerhandler", {
  ...
  handleMarkerFound: function (dishes, markerId) {
    ...
    if (dish.unavailable_days.includes(days[todaysDay])) {
      swal({
        ...
        text: "This  is toy not available today!!!",
        timer: 2500,
        buttons: false
      });
    } else {
  ...
  handleRatings: async function (toy) {
    ...
    var tNumber;
    toyNumber <= 9 ? (tNumber = `T0${toyNumber}`) : (tNumber = `T${toyNumber}` );
    
    var toySummary = await this.getOrderSummary(tNumber);
